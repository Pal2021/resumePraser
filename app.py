...
def generate(jd_text: str, resume_text: str, extra_skills: str):
    last_output = ""
    last_status = "Ready."
    try:
        for output, status in application.generate_stream(
            jd_text, resume_text, extra_skills
        ):
            if output:
                last_output = output
            if status:
...
.hero p { font-size: 1.05rem !important; color: #94a3b8 !important; margin: 0 !important; }
.gen-btn {
...
.output-card {
    ...
    margin-top: 16px !important; background: #111827 !important;
    border: 1px solid #374151 !important;
    box-shadow: 0 4px 20px rgba(0, 0, 0, 0.35) !important;
}
.output-title, .output-title * {
    color: #f9fafb !important; font-weight: 700 !important;
    font-size: 1.25rem !important; margin-bottom: 12px !important;
}
.status-pill {
...
.status-pill, .status-pill * {
    ...
    font-size: 0.98rem !important; line-height: 1.5 !important;
}
...
body:not(.dark) .output-title * { color: #0f172a !important; }
...
.gradio-container:not(.dark) .status-pill *,
body:not(.dark) .status-pill * { color: #312e81 !important; }
...
with gr.Blocks(title="Customized Resume Builder") as demo:
    with gr.Column():
        gr.Markdown(
            ...
            "Paste your target Job Description and Overleaf LaTeX resume. "
            "Get back a tailor-made, one-page, ATS-optimized `.tex` file.",
            elem_classes=["hero"],
        )
        ...
        extra_skills = gr.Textbox(
            lines=2,
            label="Additional skills to add (optional — AI skills are already included)",
            placeholder=(
                "Optional. Default AI skills are added automatically. "
                "Add any other real skills you have that aren't in your .tex yet."
            ),
        )
        ...
        with gr.Column(elem_classes=["output-card"]):
            ...
            status = gr.Markdown(
                "Ready. Paste a JD and your LaTeX resume, then click "
                "**Generate Tailored Resume**.",
        ...
        gr.ClearButton(
            value="🗑️ Clear All",
            components=[jd_text, resume_text, extra_skills, result_text, status],
        )

        generate_btn.click(
            generate,
            inputs=[jd_text, resume_text, extra_skills],
